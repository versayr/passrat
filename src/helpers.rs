...
use crate::{models::{Account, ContactInfo, Field, SecurityQuestion, Service, Shortcut}, modes::view::Detail};
...
pub fn validate_shortcut(shortcut: &Shortcut) -> Result<(), String> {
    match (shortcut.field.trim().is_empty(), shortcut.sequence.trim().is_empty()) {
        (true, true) => Err("This form is empty - please complete all fields.".to_string()),
